...
import {
  PERSONA_CATALOG,
  getPersonasByDivision,
} from "../../../../shared/personaCatalog";
...
export function AgentCard({
  ...
  const derivedEmail = teamEmail
    ? deriveAgentEmail(teamEmail, agent.slug, isLead ?? false)
    : undefined;
  ...
  const saveToken = async (): Promise<void> => {
    ...
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: ["deploy:readiness", teamSlug],
      });
    } catch (e) {
  ...
  const syncAvatar = async (): Promise<void> => {
    setTokenBusy(true);
    setTokenError(null);
    try {
            ...
            <div className="space-y-3">
              <div>
                ...
                >
                  ...
                  {Array.from(personasByDivision.entries()).map(
                    ([division, templates]) => (
            ...
            <div>
              <div className="space-y-3">
                <div>
                  <Label>
                    Role{" "}
                    <Tooltip content="Used as the 'Creature' field in identity.md and to contextualize the agent in agents.md">
                ...
                <div>
                  <Label>
                    Persona{" "}
                    <Tooltip content="Used as 'Vibe' in identity.md and as the core description in soul.md under 'Core Truths'">
                      <span className="text-gray-300 cursor-help">ⓘ</span>
                    </Tooltip>
                  </Label>
                  <Textarea
                ...
                <div>
                  <Label>
                    ...
                    <Tooltip content="Each skill becomes a bullet point in skills.md, telling the agent what it can do">
                      <span className="text-gray-300 cursor-help">ⓘ</span>
                    </Tooltip>
                  </Label>
                  <Textarea
          ...
          <div>
            ...
            <div className="space-y-2">
                ...
                <div key={mi} className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      onChange({
                        ...agent,
                        models: (agent.models ?? []).filter((_, j) => j !== mi),
                      })
                    }
                    title="Remove model"
                  >
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  onChange({ ...agent, models: [...(agent.models ?? []), ""] })
                }
              >
                + Add model
              </Button>
            ...
            <div className="space-y-3">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    set("telegramBot")(e.target.value || undefined)
                  }
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center gap-1.5">
                    <Input
                      ...
                      placeholder={
                        tokenMasked ? "Update token" : "123456:ABC..."
                      }
                    />
                    <Button
                      variant="primary"
        ...
        <>
          ...
          <div>
            ...
            <ReadField
              ...
              value={agent.role}
              full
              tooltip="Used as the 'Creature' field in identity.md and to contextualize the agent in agents.md"
            />
            <ReadField
              label="Persona"
              value={agent.persona?.trim() || undefined}
              full
              tooltip="Used as 'Vibe' in identity.md and as the core description in soul.md under 'Core Truths'"
            />
            <ReadField
              label="Skills"
              value={
          ...
          <div>
            ...
            {(agent.models ?? []).length > 0 ? (
              (agent.models ?? []).map((m, mi) => (
                <ReadField
                  key={mi}
                  label={mi === 0 ? "Primary" : `Fallback ${mi}`}
                  value={m}
                  monospace
                />
              ))
          ...
          <div>
            ...
            <ReadField
              ...
              value={agent.image}
              defaultValue={defaultImage ?? "alpine/openclaw:latest"}
            />
            <ReadField
              label="CPU (cores)"
              value={agent.cpu}
              defaultValue={defaultCpu ?? 1}
            />
            <ReadField
              label="Disk (Gi)"
              value={agent.diskGi}
              defaultValue={defaultDiskGi ?? 10}
            />
          </div>
        </>
      )}
