...
import { ChevronDown, ChevronUp, Trash2, Check, Shield } from 'lucide-react'
...
export function AgentCard({ teamSlug, agent, isFirst, providerSlugs, onChange, onDelete }: Props) {
  ...
  const saveToken = async () => {
    ...
    setTokenError(null)
    try {
      await window.api.invoke('teams:setAgentTelegramToken', { teamSlug, agentSlug: agent.slug, token: telegramToken })
  ...
  const clearToken = async () => {
    ...
    setTokenError(null)
    try {
      await window.api.invoke('teams:setAgentTelegramToken', { teamSlug, agentSlug: agent.slug, token: '' })
      setTokenMasked(null)
      setTelegramToken('')
    } catch (e) {
      setTokenError((e as Error).message)
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-gray-900 truncate">{agent.name || 'Unnamed agent'}</span>
            {isFirst && (
          ...
          <div className="text-xs text-gray-400 truncate">
            {agent.provider && <span className="font-mono">{agent.provider}</span>}
            {agent.role && <span className="ml-2">{agent.role}</span>}
          </div>
        </div>
        <div className="flex items-center gap-1 shrink-0">
          <button onClick={() => setExpanded(e => !e)} className="p-1.5 rounded-md text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors">
        ...
        <div className="px-4 pb-4 pt-1 border-t border-gray-100 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input className={inputCls} value={agent.name} onChange={e => handleNameChange(e.target.value)} placeholder="Alice" />
              {agent.slug && <p className="text-xs text-gray-400 font-mono mt-0.5">{agent.slug}</p>}
            </div>
            <div>
              <label className={labelCls}>Role</label>
              <input className={inputCls} value={agent.role} onChange={e => set('role')(e.target.value)} placeholder="Researcher" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input className={inputCls} value={agent.emoji ?? ''} onChange={e => set('emoji')(e.target.value || undefined)} placeholder="🤖" />
            </div>
            <div>
              <label className={labelCls}>Avatar URL</label>
              <input className={inputCls + ' font-mono'} value={agent.avatar ?? ''} onChange={e => set('avatar')(e.target.value || undefined)} placeholder="https://..." />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input className={inputCls + ' font-mono'} value={agent.telegramBot ?? ''} onChange={e => set('telegramBot')(e.target.value || undefined)} placeholder="123456789" />
            </div>
            <div>
              ...
              {tokenMasked && <p className="text-xs text-gray-400 font-mono mt-0.5">{tokenMasked}</p>}
              {tokenError && <p className="text-xs text-red-600 mt-0.5">{tokenError}</p>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input className={inputCls + ' font-mono'} value={agent.image ?? ''} onChange={e => set('image')(e.target.value || undefined)} placeholder="Default" />
            </div>
            <div>
