...
async function getGkeConfig(teamSlug: string): Promise<GkeDeployConfig | null> {
  ...
  const baseConfig = env.config as Partial<GkeDeployConfig>;
  const clusterZone = deployment.clusterZone ?? baseConfig.clusterZone;
  if (
  ...
  return {
    ...
    ...(baseConfig.diskZone ? { diskZone: baseConfig.diskZone } : {}),
  };
}
...
export function registerFileHandlers(): void {
  ipcMain.handle(
    "files:list",
    async (_event, teamSlug: string, agentSlug: string) => {
      const config = await getGkeConfig(teamSlug);
      if (!config) {
        return {
          files: [],
          error: "Files are only available after deployment",
        };
      }
      ...
      try {
        ...
        return {
          files: [],
          error: `Failed to fetch file list: ${e instanceof Error ? e.message : String(e)}`,
        };
      }
    },
  );
  ...
  ipcMain.handle(
    "files:get",
    async (_event, teamSlug: string, agentSlug: string, filePath: string) => {
      const config = await getGkeConfig(teamSlug);
      if (!config) {
        return {
          content: null,
      ...
      try {
        ...
        return {
          content: null,
          error: `Failed to fetch file: ${e instanceof Error ? e.message : String(e)}`,
        };
      }
    },
  );
