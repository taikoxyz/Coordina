...
import {
  ...
  getAgentLogs,
  getTeamLogs,
  resolveClusterForTeam,
} from "../environments/gke/deploy";
import {
  listGcpRegions,
  listGcpZones,
  listGkeClusters,
} from "../environments/gke/gcloud";
...
import {
  ...
  saveDeployLogs,
  clearDeployLogs,
} from "../store/deployLogs";
import { getTelegramDeployReadiness } from "../validation/telegramDeploy";

export function registerDeployHandlers(): void {
  const telegramAccount = (teamSlug: string, agentSlug: string): string =>
    `team:${teamSlug}:agent:${agentSlug}`;
  const formatValidationFailure = (
    ...
    return errors.length > 3
      ? `${prefix}: ${summary}; and ${errors.length - 3} more`
      : `${prefix}: ${summary}`;
  };
  const getDeployReadiness = async (
    teamSlug: string,
    spec: TeamSpec,
    agentSlug?: string,
  ...
  const deriveValidatedDeployFiles = async (
    teamSlug: string,
    envSlug: string,
    agentSlug?: string,
  ): Promise<{
    ...
    const [spec, env] = await Promise.all([
      getTeam(teamSlug),
    ...
    if (!deployValidation.valid) {
      throw new Error(
        formatValidationFailure(
          "Deployment file validation failed",
          deployValidation.errors,
        ),
      );
    }
  ...
  ipcMain.handle(
    ...
    async (
      _event,
      {
        ...
        spec,
        agentSlug,
      }: { teamSlug: string; spec?: TeamSpec; agentSlug?: string },
    ) => {
      const currentSpec = spec ?? (await getTeam(teamSlug));
      if (!currentSpec) return { ok: false, reason: "Team not found" };
      return getDeployReadiness(teamSlug, currentSpec, agentSlug);
    },
  );
  ...
  ipcMain.handle("gke:testAuth", async () => {
    ...
    const { clientId, clientSecret } = env.config as {
      clientId: string;
      clientSecret: string;
    };
    try {
      ...
      if (!token.token)
        return { ok: false, error: "No access token — sign in again" };
  ...
  ipcMain.handle("gcp:projects:list", async () => {
    ...
    if (!env)
      return {
        ok: false,
        projects: [],
        error: "GKE environment not configured",
      };
    const { clientId, clientSecret } = env.config as {
      clientId: string;
      clientSecret: string;
  ...
  ipcMain.handle(
    ...
    async (
      _event,
      {
        ...
        envSlug,
        agentSlug,
      }: { teamSlug: string; envSlug: string; agentSlug?: string },
    ) => {
      try {
        const { specFiles } = await deriveValidatedDeployFiles(
          teamSlug,
          envSlug,
          agentSlug,
        );
        const filteredFiles = agentSlug
          ? specFiles.filter(
              (f) =>
                !f.path.includes("/") ||
                f.path.startsWith(`agents/${agentSlug}/`),
            )
          : specFiles;
        return {
          ok: true,
  ...
  ipcMain.handle(
    ...
    async (
      ...
      let specFiles;
      try {
        const prepared = await deriveValidatedDeployFiles(
          teamSlug,
          envSlug,
          agentSlug,
        );
      ...
      const deployConfig = {
        slug: envSlug,
        ...(env.config as object),
      } as Parameters<typeof deployTeam>[2];

      let resolved: { clusterName: string; clusterZone: string };
      try {
        resolved = await resolveClusterForTeam(
          teamSlug,
          {
          ...
          (msg) =>
            win?.webContents.send("deploy:status", {
              resource: "Cluster",
              status: "exists",
              message: msg,
            }),
        ...
        return {
          ok: false,
          reason: `Cluster resolution failed: ${e instanceof Error ? e.message : String(e)}`,
        };
      }

      try {
        for await (const status of deployTeam(
          filesToDeploy,
          teamSlug,
          deployConfig,
          options,
        ...
        if (leadAgent) {
          if (
            mode === "ingress" &&
            (!envDomain || envDomain.trim().length === 0)
          ) {
            return {
              ok: false,
              reason:
                "Environment domain is required when gateway mode is ingress",
            };
          }
  ...
  ipcMain.handle(
    ...
    async (
      event,
      {
        ...
        envSlug,
        deleteDisks,
      }: { teamSlug: string; envSlug: string; deleteDisks?: boolean },
    ) => {
      const env = await getEnvironment(envSlug);
      if (!env) return { ok: false, reason: "Environment not found" };
      ...
      const deployConfig = {
        slug: envSlug,
        ...(env.config as object),
      } as Parameters<typeof undeployTeam>[1];
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const deployConfig = {
        slug: envSlug,
        ...(env.config as object),
      } as Parameters<typeof undeployAgent>[2];
      ...
      try {
        for await (const status of undeployAgent(
          teamSlug,
          agentSlug,
          deployConfig,
          { deleteDisks },
  ...
  ipcMain.handle(
    "deploy:getLogs",
    async (_e, { teamSlug }: { teamSlug: string }) => {
  ...
  ipcMain.handle(
    ...
    async (
      _e,
      { teamSlug, entries }: { teamSlug: string; entries: unknown[] },
    ) => {
      await saveDeployLogs(
        teamSlug,
        entries as Parameters<typeof saveDeployLogs>[1],
      );
      return { ok: true };
    },
  );
  ...
  ipcMain.handle(
    ...
    async (
      _e,
      { teamSlug, envSlug }: { teamSlug: string; envSlug: string },
    ) => {
      const [spec, env] = await Promise.all([
        getTeam(teamSlug),
        getEnvironment(envSlug),
      ...
      const deployConfig = {
        slug: envSlug,
        ...(env.config as object),
      } as Parameters<typeof getTeamStatus>[2];
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (!spec || !env)
        return { ok: false, reason: "Team or environment not found" };
      const deployConfig = {
        slug: envSlug,
        ...(env.config as object),
      } as Parameters<typeof getTeamStatus>[2];
      ...
      try {
        const entries = await getTeamLogs(
          teamSlug,
          spec.agents.map((a) => a.slug),
          deployConfig,
          opts,
