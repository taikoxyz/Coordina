...
export function generateToolsMd(input: ToolsInput): string {
  ...
  if (input.hasEmail && input.agentEmail) {
    lines.push(
      ...
      '',
      'Use the Google Workspace CLI (`gws`) to read and send email.',
      '',
      '### Reading email',
      '```bash',
      'gws gmail messages list --user-id me --q "to:' + input.agentEmail + ' is:unread" --format json',
      '```',
      '',
      '### Reading a specific message',
      '```bash',
      'gws gmail messages get --user-id me --id <MESSAGE_ID> --format json',
      '```',
      '',
      '### Sending email',
      '```bash',
      `gws gmail messages send --user-id me --to "<recipient>" --subject "<subject>" --body "<body>" --from "${input.agentEmail}"`,
      '```',
      '',
      '### Email rules',
