...
describe("generateAgentStatefulSet", () => {
  it("generates StatefulSet manifest with deterministic PVC name", () => {
    const manifest = generateAgentStatefulSet({
      teamSlug: "eng-alpha",
      agentSlug: "alice",
    });
  ...
  it("mounts credential secret at openclaw-state/openclaw.json when provided", () => {
    const manifest = generateAgentStatefulSet({
      teamSlug: "eng-alpha",
      agentSlug: "alice",
      credentialSecretName: "eng-alpha-anthropic-credentials",
    });
  ...
  it("init container creates state and workspace directories", () => {
    ...
    expect(manifest).toContain(
      "mkdir -p /agent-data/openclaw/state /agent-data/openclaw/workspace",
    );
  ...
  it("sets default memory request and limit to 2.5Gi", () => {
    const manifest = generateAgentStatefulSet({
      teamSlug: "eng-alpha",
      agentSlug: "alice",
    });
    expect(manifest).toContain("memory: 2.5Gi");
  });
...
describe("generateIngress", () => {
  it("generates Ingress with paths for each agent", () => {
    const manifest = generateIngress({
      teamSlug: "eng-alpha",
      agents: ["alice", "bob"],
      domain: "example.com",
    });
...
describe("generateAgentService", () => {
  it("adds NEG annotation required by GCE ingress for ClusterIP backends", () => {
    const manifest = generateAgentService({
      teamSlug: "eng-alpha",
      agentSlug: "alice",
    });
...
describe("generateAgentConfigMap", () => {
  it("generates per-agent ConfigMap with agent markdown files", () => {
    const yaml = generateAgentConfigMap({
      ...
      toolsMd: "# Tools",
      openclawJson:
        '{ "agents": { "defaults": { "model": { "primary": "anthropic/claude-sonnet-4-6" } } }, "models": { "providers": { "anthropic": {} } } }',
    });
...
describe("generateAgentPvc", () => {
  it("generates PVC with dynamic StorageClass instead of static volumeName", () => {
    const manifest = generateAgentPvc({
      teamSlug: "eng-alpha",
      agentSlug: "alice",
      namespace: "eng-alpha",
    });
